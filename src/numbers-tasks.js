...
function getAverage(value1, value2) {
  ...
  return average >= Number.MAX_VALUE ? Number.MAX_VALUE : average;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const bottom = Math.sqrt(x1 ** 2 + x2 ** 2) * Math.sqrt(y1 ** 2 + y2 ** 2);
  if (Math.abs(bottom) < 1e-10) {
...
function toNumber(value, def) {
  return parseInt(value, 10) === +value ? +value : def;
}
